...
const template = "https://host.cc.ntu.edu.tw/activities/placeApplyDetail.aspx?From=placeApply.aspx&Place_ID={place}&Act_ID={event}&Start={start}&End={end}&Date={date}";
...
const generate = function(event_ID, place_order, dates){
	let template_with_event = template.format({event: event_ID});
	for(let i = 0;i < dates.length;i++){
		let date = dates[i];
		let date_string = give_datestring(date);
		let date_8char = give_date8char(date);
		let template_with_event_date = template_with_event.format({date: date_string});
		...
		for(let j = 0;j < place_order.length;j++){
			let place = place_order[j];
			let place_info = place_map[place];
			let address = template_with_event_date.format({place: place_info['id'], start: place_info['start'], end: place_info['end']});
			...
			let ID = "button{0}{1}".format(date_8char, place);
...
const test_generate = function(event_ID, place_order, dates){
	document.getElementById("testActID").value = event_ID;
	if(place_order.length > 0){
		let node = document.getElementById("testPlace");
		let place_info_0 = place_map[place_order[0]];
	...
	new ClipboardJS(document.getElementById("testButton"), {
		text: function(trigger) {
			let place_info = place_map[document.getElementById("testPlace").value];
...
const onChangeOfTestPlaceID = function(){
	let place_info = place_map[document.getElementById("testPlace").value];
